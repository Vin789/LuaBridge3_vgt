...
inline Options get_class_options(lua_State* L, int index)
{
    ...
    lua_rawgetp_vgt(L, index, getClassOptionsKey()); // Stack: mt, ifb (may be nil)
    if (lua_isnumber(L, -1))
...
inline void push_class_or_const_table(lua_State* L, int index)
{
    ...
    lua_rawgetp_vgt(L, index, getClassKey()); // Stack: mt, class table (ct) | nil
    if (! lua_istable(L, -1)) // Stack: mt, nil
    {
        lua_pop(L, 1); // Stack: mt

        lua_rawgetp_vgt(L, index, getConstKey()); // Stack: mt, const table (co) | nil
        if (! lua_istable(L, -1)) // Stack: mt, nil
            return;
    }
}
...
inline std::optional<int> try_call_index_fallback(lua_State* L)
{
    ...
    lua_rawgetp_vgt(L, -1, getIndexFallbackKey()); // Stack: mt, ifb (may be nil)
...
inline std::optional<int> try_call_index_extensible(lua_State* L, const char* key)
{
    ...
    if constexpr (IsObject)
        ...
        lua_rawgetp_vgt(L, -1, getStaticKey()); // Stack: mt, st
...
inline int index_metamethod(lua_State* L)
{
    ...
    for (;;)
    {
        ...
        lua_rawgetp_vgt(L, -1, getPropgetKey()); // Stack: mt, propget table (pg)
        LUABRIDGE_ASSERT(lua_istable(L, -1));
        ...
        lua_rawgetp_vgt(L, -1, getParentKey()); // Stack: mt, parent mt | nil
...
inline std::optional<int> try_call_newindex_fallback(lua_State* L)
{
    ...
    lua_rawgetp_vgt(L, -1, getNewIndexFallbackKey()); // Stack: mt, nifb (may be nil)
    if (! lua_iscfunction(L, -1))
    {
        lua_pop(L, 1); // Stack: mt
        return std::nullopt;
...
inline std::optional<int> try_call_newindex_extensible(lua_State* L, const char* key)
{
    ...
    for (;;)
    {
        ...
        lua_rawgetp_vgt(L, -2, getParentKey()); // Stack: mt, mt, ct | co, parent mt (pmt) | nil
...
inline int newindex_metamethod(lua_State* L)
{
    ...
    for (;;)
    {
        ...
        lua_rawgetp_vgt(L, -1, getPropsetKey()); // Stack: mt, propset table (ps) | nil
        if (lua_isnil(L, -1)) // Stack: mt, nil
...
int tostring_metamethod(lua_State* L)
{
    ...
    lua_getmetatable(L, -1); // Stack: metatable (mt)
    lua_rawgetp_vgt(L, -1, getTypeKey()); // Stack: mt, classname (cn)
...
int destruct_metamethod(lua_State* L)
{
    ...
    lua_rawgetp_vgt(L, LUA_REGISTRYINDEX, detail::getClassRegistryKey<C>()); // Stack: ud, ot, registry metatable (rt) | nil
...
inline void add_property_getter(lua_State* L, const char* name, int tableIndex)
{
    ...
    lua_rawgetp_vgt(L, tableIndex, getPropgetKey()); // Stack: getter, propget table (pg)
...
inline void add_property_setter(lua_State* L, const char* name, int tableIndex)
{
    ...
    lua_rawgetp_vgt(L, tableIndex, getPropsetKey()); // Stack: setter, propset table (ps)
