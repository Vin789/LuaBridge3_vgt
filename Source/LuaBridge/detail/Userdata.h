...
private:
                              ...
                              bool canBeConst)
    {
        ...
        {
            lua_rawgetp_vgt(L, LUA_REGISTRYINDEX, registryClassKey); // Stack: ot | nil, registry metatable (rt) | nil
            return throwBadArg(L, index);
        }

        lua_rawgetp_vgt(L, -1, getConstKey()); // Stack: ot | nil, const table (co) | nil
        ...
        lua_rawgetp_vgt(L, LUA_REGISTRYINDEX, (isConst && canBeConst)
            ? registryConstKey
        ...
        for (;;)
        {
            ...
            lua_rawgetp_vgt(L, -1, getParentKey()); // Stack: rt, ot, parent ot (pot) | nil
    ...
    static bool isInstance(lua_State* L, int index, const void* registryKey)
    {
        ...
        lua_rawgetp_vgt(L, LUA_REGISTRYINDEX, registryKey); // Stack: ot, rt
    ...
    static Userdata* throwBadArg(lua_State* L, int index)
    {
        ...
        {
            lua_rawgetp_vgt(L, -1, getTypeKey()); // Stack: rt, registry type
        ...
        if (lua_isuserdata(L, index))
        {
            ...
            {
                lua_rawgetp_vgt(L, -1, getTypeKey()); // Stack: rt, ot, object type | nil
                if (lua_isstring(L, -1))
...
public:
    ...
    static UserdataValue<T>* place(lua_State* L, std::error_code& ec)
    {
        ...
        lua_rawgetp_vgt(L, LUA_REGISTRYINDEX, detail::getClassRegistryKey<T>());
...
private:
    ...
    static Result push(lua_State* L, const void* ptr, const void* key)
    {
        ...
        lua_rawgetp_vgt(L, LUA_REGISTRYINDEX, key);
...
struct UserdataSharedHelper
{
    ...
    static Result push(lua_State* L, const C& c)
    {
        if (ContainerTraits<C>::get(c) != nullptr)
        {
            ...
            lua_rawgetp_vgt(L, LUA_REGISTRYINDEX, getClassRegistryKey<T>());
...
struct UserdataSharedHelper<C, true>
{
    ...
    static Result push(lua_State* L, const C& c)
    {
        if (ContainerTraits<C>::get(c) != nullptr)
        {
            ...
            lua_rawgetp_vgt(L, LUA_REGISTRYINDEX, getConstRegistryKey<T>());
